...
export function DreamTeam({ setHeaderActions }: DreamTeamProps) {
  ...
  const [team, setTeam] = useState<ConfiguredAIProfile[]>(() => 
      DEFAULT_AI_PROFILES.map(p => ({
          ...
          model: p.avatar === 'gpt' ? 'gpt-4o' : p.avatar === 'claude' ? 'claude-3-5-sonnet-20241022' : 'gemini-2.5-pro'
      }))
  );
    ...
    if (statusRef.current === 'running') {
      ...
      if (nextTurn >= team.length) {
        ...
        if (nextCycle >= targetCycles) {
          // Only auto-summarize in autonomous mode
          // In manual mode, pause so user can manually trigger summary when ready
  ...
  const handleSaveProfile = (updatedProfile: ConfiguredAIProfile) => {
    if (editingProfile && team.find(p => p.id === editingProfile.id)) {
        // Editing an existing profile - update it
